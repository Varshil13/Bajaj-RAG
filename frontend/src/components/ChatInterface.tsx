...
const ChatInterface: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'ECONNREFUSED') {
        errorMessage = 'Unable to connect to the RAG server. Please make sure your backend is running on localhost:3001.';
      } else if (err.response?.status === 404) {
        errorMessage = 'The API endpoint was not found. Please check your RAG backend configuration.';
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden" style={{
      ...
      {/* Chat Messages */}
      <div className="h-96 overflow-y-auto p-6 space-y-4" style={{
        overflowY: 'auto',
        padding: '1rem',
        display: 'flex',
        flexDirection: 'column',
        ...
        {messages.map((message) => (
          ...
          >
            {!message.isUser && (
              <div className="flex-shrink-0 w-8 h-8 bg-primary-600 rounded-full flex items-center justify-center" style={{
                flexShrink: 0,
                width: '2rem',
                height: '2rem',
            ...
            >
              ...
              {!message.isUser && message.claimResult ? (
                <div className="space-y-3">
                  <div className={`inline-block px-3 py-1 rounded-full text-sm font-semibold ${
                    ...
                    backgroundColor: message.claimResult.Decision?.toLowerCase() === 'approved' ? '#dcfce7' : '#fef2f2',
                    color: message.claimResult.Decision?.toLowerCase() === 'approved' ? '#166534' : '#dc2626',
                    border: `1px solid ${message.claimResult.Decision?.toLowerCase() === 'approved' ? '#bbf7d0' : '#fecaca'}`,
                  ...
                  {message.claimResult.Amount && (
                    <div className="bg-blue-50 p-3 rounded-lg border border-blue-200" style={{
                      backgroundColor: '#1e40af20',
                      padding: '0.75rem',
                      borderRadius: '0.5rem',
                      border: '1px solid #3b82f6'
                    }}>
                      <div className="text-sm font-medium text-blue-900" style={{ color: '#60a5fa', fontWeight: '500' }}>
                        💰 Coverage Amount
                      </div>
                      <div className="text-lg font-bold text-blue-800" style={{ color: '#93c5fd', fontSize: '1.125rem', fontWeight: 'bold' }}>
                  ...
                  <div className="space-y-2">
                    <div className="text-sm font-medium" style={{ fontWeight: '500', color: '#cbd5e1' }}>
                      📋 Justification:
                    </div>
              ...
              <p className={`text-xs mt-2 ${
                ...
                {!message.isUser && message.type === 'claim' && (
                  <span className="ml-2 px-2 py-0.5 bg-blue-500 text-white rounded text-xs" style={{
                    marginLeft: '0.5rem',
            ...
            {message.isUser && (
              <div className="flex-shrink-0 w-8 h-8 bg-gray-600 rounded-full flex items-center justify-center" style={{
                flexShrink: 0,
                width: '2rem',
                height: '2rem',
                ...
                <User className="w-4 h-4 text-white" style={{ color: 'white' }} />
              </div>
            )}
          </div>
        ))}
        ...
        {isLoading && (
          <div className="flex items-start space-x-3 justify-start animate-slide-up" style={{
            display: 'flex',
            alignItems: 'flex-start',
            gap: '0.75rem',
            ...
            <div className="bg-gray-100 text-gray-900 px-4 py-3 rounded-lg" style={{
      ...
      {/* Error Display */}
      {error && (
        <div className="px-6 py-3 bg-red-50 border-t border-red-200" style={{
          padding: '0.75rem 1.5rem',
          backgroundColor: '#7f1d1d',
          borderTop: '1px solid #991b1b'
        }}>
          <div className="flex items-center space-x-2 text-red-700" style={{
            display: 'flex',
            alignItems: 'center',
            gap: '0.5rem',
            color: '#fecaca'
      ...
      {/* Input Form */}
      <div className="border-t border-gray-200 p-4" style={{
        borderTop: '1px solid #334155',
        padding: '0.75rem',
        backgroundColor: '#1e293b',
        flexShrink: 0
      }}>
        <form onSubmit={handleSubmit} className="flex space-x-3" style={{
          display: 'flex',
          gap: '0.75rem'
        }}>
          <input
          ...
          <button
            ...
            style={{
              padding: '0.5rem 1rem',
              backgroundColor: !inputText.trim() || isLoading ? '#475569' : '#3b82f6',
              color: 'white',
              borderRadius: '0.5rem',
              border: 'none',
